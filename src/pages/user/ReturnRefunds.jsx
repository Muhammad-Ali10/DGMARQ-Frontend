...
import { Plus, Eye, DollarSign, X } from 'lucide-react';
...
const UserReturnRefunds = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => returnRefundAPI.createRefundRequest(data),
    onSuccess: () => {
      queryClient.invalidateQueries(['user-refunds']);
      setIsCreateOpen(false);
      setFormData({ orderId: '', productId: '', reason: '' });
      showSuccess('Refund request created successfully');
    },
    onError: (error) => {
      showApiError(error, 'Failed to create refund request');
    },
  });
      ...
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          ...
          <DialogContent className="bg-primary border-gray-700">
            <DialogHeader>
              <DialogTitle className="text-white">Create Refund Request</DialogTitle>
            </DialogHeader>
                        ...
                        <div className="flex gap-2">
                          ...
                          {refund.status === 'pending' && (
                            <Button
                              ...
                              variant="destructive"
                              onClick={() => {
                                  cancelMutation.mutate(refund._id);
                              }}
                            >
                              <X className="w-4 h-4" />
                            </Button>
