...
const TypesManagement = () => {
  ...
  const {
    ...
    queryKey: ["types", page, search, isActiveFilter],
    queryFn: () => {
      const params = { page, limit: 10 };
      if (search.trim()) params.search = search.trim();
      if (isActiveFilter !== "") params.isActive = isActiveFilter;
      return typeAPI.getAllTypes(params).then((res) => res.data.data);
    },
    keepPreviousData: true,
  });

  const types = typesData?.docs || typesData?.types || [];
  const pagination = typesData
    ? {
        page: typesData.page || 1,
        totalPages: typesData.totalPages || 1,
        totalDocs: typesData.totalDocs || 0,
        limit: typesData.limit || 10,
        hasNextPage: typesData.hasNextPage || false,
        hasPrevPage: typesData.hasPrevPage || false,
      }
    : {
        page: 1,
        totalPages: 1,
  ...
  const handleCreate = (e) => {
    ...
    if (!formData.name?.trim()) {
      toast.warning("Product type name is required");
      return;
    }
  ...
  if (isLoading && !typesData)
    return <Loading message="Loading product types..." />;
  if (isError)
    return (
      <ErrorMessage
        message={
          error?.response?.data?.message || "Error loading product types"
        }
      />
    );
    ...
    <div className="space-y-6 px-4 sm:px-0">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm sm:text-base text-gray-400 mt-1">Manage product types</p>
        </div>
                ...
                {types && types.length > 0 ? (
                  types.map((type) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge
                          variant={type.isActive ? "success" : "destructive"}
                      ...
                      <TableCell>
                        <div className="flex gap-2 justify-end">
                          ...
                          <Button
                            ...
                            variant="outline"
                            onClick={() =>
                              toggleStatusMutation.mutate(type._id)
                            }
                            className={`border-gray-700 ${
                              type.isActive
