...
import { Plus, Edit, Trash2, ChevronLeft, ChevronRight, Search, X, RefreshCw } from 'lucide-react';
...
const RegionsManagement = () => {
  ...
  // Extract regions array and pagination info from response
  // Region backend uses aggregatePaginate, returns { docs, totalDocs, page, totalPages, ... }
  const regions = regionsData?.docs || regionsData?.regions || [];
  const pagination = regionsData ? {
    page: regionsData.page || 1,
    totalPages: regionsData.totalPages || 1,
    totalDocs: regionsData.totalDocs || 0,
    limit: regionsData.limit || 10,
    hasNextPage: regionsData.hasNextPage || false,
    hasPrevPage: regionsData.hasPrevPage || false,
  } : {
    page: 1,
    totalPages: 1,
    totalDocs: 0,
  ...
  const handleCreate = (e) => {
    ...
    if (!formData.name?.trim()) {
      toast.warning('Region name is required');
      return;
    }
  ...
  if (isLoading && !regionsData) return <Loading message="Loading regions..." />;
    ...
    <div className="space-y-6 px-4 sm:px-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm sm:text-base text-gray-400 mt-1">Manage game regions</p>
        </div>
              ...
              <p className="text-sm text-gray-400 mt-1">
                {pagination.totalDocs > 0 ? (
                  <>
                    ...
                    <span className="text-white font-medium">{pagination.totalDocs}</span> regions
                  </>
                ) : (
                  'No regions found'
                )}
            ...
            <Table>
              ...
              <TableBody>
                {regions && regions.length > 0 ? (
                  regions.map((region) => (
                    <TableRow key={region._id} className="border-gray-700 hover:bg-secondary/20">
                      <TableCell className="font-semibold text-white">{region.name}</TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-2 justify-end">
                          ...
                          <Button
                            ...
                            variant="destructive"
                            onClick={() => {
                                deleteMutation.mutate(region._id);
                            }}
