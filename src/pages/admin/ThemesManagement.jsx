...
const ThemesManagement = () => {
  ...
  const {
    ...
    queryKey: ["themes", page, search],
    queryFn: () => {
      const params = { page, limit: 10 };
      if (search.trim()) params.search = search.trim();
      return themeAPI.getThemes(params).then((res) => res.data.data);
    },
    keepPreviousData: true,
  });

  const themes = themesData?.docs || themesData?.themes || [];
  const pagination = themesData
    ? {
        page: themesData.page || 1,
        totalPages: themesData.totalPages || 1,
        totalDocs: themesData.totalDocs || 0,
        limit: themesData.limit || 10,
        hasNextPage: themesData.hasNextPage || false,
        hasPrevPage: themesData.hasPrevPage || false,
      }
    : {
        page: 1,
        totalPages: 1,
  ...
  const handleCreate = (e) => {
    ...
    if (!formData.name?.trim()) {
      toast.warning("Theme name is required");
      return;
    }
  ...
  if (isLoading && !themesData) return <Loading message="Loading themes..." />;
  if (isError)
    return (
      <ErrorMessage
        message={error?.response?.data?.message || "Error loading themes"}
      />
    );
  ...
  return (
    <div className="space-y-6 px-4 sm:px-0">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white">Themes Management</h1>
          <p className="text-sm sm:text-base text-gray-400 mt-1">Manage game themes</p>
        </div>
