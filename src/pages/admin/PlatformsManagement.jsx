...
import { Plus, Edit, Trash2, Power, ChevronLeft, ChevronRight, Filter, RefreshCw } from 'lucide-react';
...
const PlatformsManagement = () => {
  ...
  const { data: platformsData, isLoading, isError, error } = useQuery({
    queryKey: ['platforms', page, isActiveFilter],
    queryFn: () => {
      const params = { page, limit: 10 };
      if (isActiveFilter !== '') params.isActive = isActiveFilter;
      return platformAPI.getAllPlatforms(params).then(res => res.data.data);
    },
    keepPreviousData: true,
  });

  // Extract platforms array and pagination info from response
  // Platform backend returns { total, page, limit, platforms }
  const platforms = platformsData?.platforms || [];
  const pagination = platformsData ? {
    page: platformsData.page || 1,
    totalPages: Math.ceil((platformsData.total || 0) / (platformsData.limit || 10)),
    totalDocs: platformsData.total || 0,
    limit: platformsData.limit || 10,
    hasNextPage: (platformsData.page || 1) * (platformsData.limit || 10) < (platformsData.total || 0),
    hasPrevPage: (platformsData.page || 1) > 1,
  } : {
    page: 1,
    totalPages: 1,
    totalDocs: 0,
  ...
  const handleCreate = (e) => {
    ...
    if (!formData.name?.trim()) {
      toast.warning('Platform name is required');
      return;
    }
  ...
  if (isLoading && !platformsData) return <Loading message="Loading platforms..." />;
  if (isError) return <ErrorMessage message={error?.response?.data?.message || "Error loading platforms"} />;
  ...
  return (
    <div className="space-y-6 px-4 sm:px-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white">Platforms Management</h1>
          <p className="text-sm sm:text-base text-gray-400 mt-1">Manage gaming platforms</p>
        </div>
      ...
      <Card className="bg-primary border-gray-700 shadow-xl">
        <CardHeader className="border-b border-gray-700">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <CardTitle className="text-white text-xl font-semibold">All Platforms</CardTitle>
              <p className="text-sm text-gray-400 mt-1">
                {pagination.totalDocs > 0 ? (
                  <>
                    ...
                    <span className="text-white font-medium">{pagination.totalDocs}</span> platforms
                  </>
                ) : (
                  'No platforms found'
                )}
              ...
              <TableBody>
                {platforms && platforms.length > 0 ? (
                  platforms.map((platform) => (
                    <TableRow key={platform._id} className="border-gray-700 hover:bg-secondary/20">
                      <TableCell className="font-semibold text-white">{platform.name}</TableCell>
                      <TableCell>
                        <Badge 
                          variant={platform.isActive ? 'success' : 'destructive'}
                      ...
                      <TableCell className="text-gray-400">
                        {platform.createdAt ? new Date(platform.createdAt).toLocaleDateString() : '-'}
                      </TableCell>
                      <TableCell>
                        <div className="flex gap-2 justify-end">
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => toggleStatusMutation.mutate(platform._id)}
                            className={`border-gray-700 ${platform.isActive ? 'hover:bg-orange-600/20' : 'hover:bg-green-600/20'}`}
                          ...
                          <Button
                            ...
                            variant="destructive"
                            onClick={() => {
                                deleteMutation.mutate(platform._id);
                            }}
                  ...
                  <TableRow>
                    <TableCell colSpan={4} className="text-center py-12">
                      <div className="flex flex-col items-center justify-center gap-3">
                        <p className="text-gray-400 font-medium">No platforms found</p>
