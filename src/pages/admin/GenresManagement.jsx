...
const GenresManagement = () => {
  ...
  const {
    ...
    queryFn: () => {
      ...
      return genreAPI.getGenres(params).then((res) => res.data.data);
    },
    keepPreviousData: true,
  });

  // Extract genres array and pagination info from response
  // Genre backend uses aggregatePaginate, returns { docs, totalDocs, page, totalPages, ... }
  const genres = genresData?.docs || genresData?.genres || [];
  const pagination = genresData
    ? {
        page: genresData.page || 1,
        totalPages: genresData.totalPages || 1,
        totalDocs: genresData.totalDocs || 0,
        limit: genresData.limit || 10,
        hasNextPage: genresData.hasNextPage || false,
        hasPrevPage: genresData.hasPrevPage || false,
      }
    : {
        page: 1,
        totalPages: 1,
  ...
  const handleCreate = (e) => {
    ...
    if (!formData.name?.trim()) {
      toast.warning("Genre name is required");
      return;
    }
  ...
  if (isLoading && !genresData) return <Loading message="Loading genres..." />;
  if (isError)
    return (
      <ErrorMessage
        message={error?.response?.data?.message || "Error loading genres"}
      />
    );
  ...
  return (
    <div className="space-y-6 px-4 sm:px-0">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white">Genres Management</h1>
          <p className="text-sm sm:text-base text-gray-400 mt-1">Manage game genres</p>
        </div>
                      ...
                      <TableCell>
                        <div className="flex gap-2 justify-end">
                          ...
                          <Button
                            ...
                            variant="destructive"
                            onClick={() => {
                              deleteMutation.mutate(genre._id);
                            }}
